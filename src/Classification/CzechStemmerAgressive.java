 ...
 */
public class CzechStemmerAgressive implements Serializable {
	/**
...
public CzechStemmerAgressive(){} // constructor
	...
	private void removeDerivational(StringBuffer buffer) {
		...
		if(len > 7){
			if(buffer.substring( len-5 ,len).equals("ion\u00e1\u0159")){ // -ionáø 
		...
		if(len > 5){
			...
			if(buffer.substring( len-3 ,len).equals("\u011bnk")){ //-ìnk
			...
			if(buffer.substring( len-3 ,len).equals("kyn")|| 
					buffer.substring( len-3 ,len).equals("\u010dan")||    //-èan
				    buffer.substring( len-3 ,len).equals("k\u00e1\u0159")|| //káø
				    buffer.substring( len-3 ,len).equals("n\u00e9\u0159")|| //néø
		 ...
		 if(len > 4){
			if(buffer.substring( len-2 ,len).equals("\u00e1\u010d")|| // -áè
				buffer.substring( len-2 ,len).equals("a\u010d")||      //-aè
				buffer.substring( len-2 ,len).equals("\u00e1n")||      //-án
			        buffer.substring( len-2 ,len).equals("an")|| 
			        buffer.substring( len-2 ,len).equals("\u00e1\u0159")|| //-áø
			...
			if(buffer.substring( len-2 ,len).equals("ec")|| 
				    ...
				    buffer.substring( len-2 ,len).equals("\u011bn")||   //-ìn
				    buffer.substring( len-2 ,len).equals("\u00e9\u0159")){  //-éø
						
				    buffer.delete( len-1 , len);
				    palatalise(buffer);
				    return;
			}
			if(buffer.substring( len-2 ,len).equals("\u00ed\u0159")|| //-íø
			...
			if(buffer.substring( len-2 ,len).equals("ob")|| 
				    ...
				    buffer.substring( len-2 ,len).equals("o\u0148")){ //-oò 
			...
			if(buffer.substring( len-2 ,len).equals("\u010dk")||              //-èk
				    buffer.substring( len-2 ,len).equals("\u010dn")||  //-èn
		 ...
		 if(len > 3){
				if(buffer.charAt(buffer.length()-1)=='c'||
				   buffer.charAt(buffer.length()-1)=='\u010d'|| //-è
	...
	private void removeDiminutive(StringBuffer buffer) {
		...
		if( len> 6){
			 if(buffer.substring( len-4,len).equals("e\u010dek")||      //-eèek
			    buffer.substring( len-4,len).equals("\u00e9\u010dek")||    //-éèek
			    buffer.substring( len-4,len).equals("i\u010dek")||         //-ièek
			    buffer.substring( len-4,len).equals("\u00ed\u010dek")||    //íèek
			    ...
			    return;
		            ...
		            if( buffer.substring( len-4,len).equals("\u00e1\u010dek")|| //áèek
		                 buffer.substring( len-4,len).equals("a\u010dek")||   //aèek
		        	     buffer.substring( len-4,len).equals("o\u010dek")||   //oèek
		        	     buffer.substring( len-4,len).equals("u\u010dek")||   //uèek
		...
		if( len> 5){
			    if(buffer.substring( len-3,len).equals("e\u010dk")||   //-eèk
			       buffer.substring( len-3,len).equals("\u00e9\u010dk")||  //-éèk 
			       buffer.substring( len-3,len).equals("i\u010dk")||   //-ièk
			       buffer.substring( len-3,len).equals("\u00ed\u010dk")||    //-íèk
			    ...
			    if(buffer.substring( len-3,len).equals("\u00e1\u010dk")||  //-áèk
			        buffer.substring( len-3,len).equals("au010dk")|| //-aèk
			        buffer.substring( len-3,len).equals("o\u010dk")||  //-oèk
			        buffer.substring( len-3,len).equals("u\u010dk")||   //-uèk 
	...
	private void removeComparative(StringBuffer buffer) {
		...
		if( (len> 5)&&
			(buffer.substring( len-3,len).equals("ej\u0161")||  //-ejš
			 buffer.substring( len-3,len).equals("\u011bj\u0161"))){   //-ìjš
	...
	private void palatalise(StringBuffer buffer){
		...
		if( buffer.substring( len- 2 ,len).equals("ci")||
		     ...
		     buffer.substring( len- 2 ,len).equals("\u010di")||      //-èi
		     buffer.substring( len- 2 ,len).equals("\u010de")){   //-èe
		...
		if( buffer.substring( len- 3 ,len).equals("\u010dt\u011b")||     //-ètì
		     buffer.substring( len- 3 ,len).equals("\u010dti")||   //-èti
		     buffer.substring( len- 3 ,len).equals("\u010dt\u00ed")){   //-ètí
						
		     buffer.replace(len- 3 ,len, "ck");
		     return;
		}
		if( buffer.substring( len- 2 ,len).equals("\u0161t\u011b")||   //-štì
	...
	private void removePossessives(StringBuffer buffer) {
		...
		if( len> 5 ){
			...
			if(buffer.substring( len-2,len).equals("\u016fv")){ //-ùv
	...
	private void removeCase(StringBuffer buffer) {
		...
		if( len> 6 ){
		      if(buffer.substring( len- 4 ,len).equals("\u011btem")){   //-ìtem
		
		         buffer.delete( len- 3 , len);
		         palatalise(buffer);
		         return;
		      }
		       if(buffer.substring( len- 4 ,len).equals("at\u016fm")){  //-atùm
		    	      buffer.delete( len- 4 , len);
		    	      return;
		      }
		...
		if( len> 5 ){
				...
				buffer.substring( len-3,len).equals("\u00edch")){ //-ích
				  ...
				  return;
				    ...
				    buffer.substring( len-3,len).equals("\u011bmi")||  //-ìmu
		...
		if( len> 3){
			 if(buffer.substring( len-1,len).equals("e")||
			    ...
			    buffer.substring( len-1,len).equals("\u011b")){   //-ì
			     ...
			     buffer.substring( len-1,len).equals("\u016f")){   //-ù
